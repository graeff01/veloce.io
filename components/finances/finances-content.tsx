...
import {
  ...
  Loader2, Repeat, Users, CheckCircle2, Clock, AlertCircle,
  ChevronDown,
} from "lucide-react";
...
const STATUS_CONFIG = {
  PAGO:     { label: "Pago",     color: "#16A34A", bg: "rgba(22,163,74,0.12)",  icon: CheckCircle2 },
  PENDENTE: { label: "Pendente", color: "#D97706", bg: "rgba(217,119,6,0.12)",  icon: Clock        },
  VENCIDO:  { label: "Vencido",  color: "#DC2626", bg: "rgba(220,38,38,0.12)",  icon: AlertCircle  },
};
...
export function FinancesContent() {
  ...
  // entry + current status for the status modal
  const [statusModal, setStatusModal] = useState<{ entry: Entry } | null>(null);
  ...
  const teamEntries: Entry[] = team
    ...
    .flatMap((p): Entry[] => {
      ...
      if (p.type === "PRESTADOR" && p.unitValue) {
        ...
        if (qty === 0) return [];
        return [{
          id: `team-${p.id}`, type: "DESPESA", mode: "AVULSO",
          description: p.name, category: "Pagamento PJ",
          value: qty * p.unitValue, date: `${monthPrefix}-05`,
          status: statusOverrides[`team-${p.id}`] ?? "PENDENTE",
          client: null, notes: `${qty}× ${p.unit || "entrega"} · ${p.role || ""}`.trim(),
        }];
      }
      if (p.salary > 0) {
        return [{
          id: `team-${p.id}`, type: "DESPESA", mode: "RECORRENTE",
          description: p.name,
          category: p.type === "FUNCIONARIO" ? "Salário CLT" : "Pagamento PJ",
          value: p.salary, date: `${monthPrefix}-05`,
          status: statusOverrides[`team-${p.id}`] ?? "PENDENTE",
          client: null, notes: p.role || null,
        }];
      }
      return [];
    });
  ...
  // Sorted: VENCIDO first, then PENDENTE, then PAGO — within each group by value desc
  const ORDER: Record<Entry["status"], number> = { VENCIDO: 0, PENDENTE: 1, PAGO: 2 };
  const sortedFiltered = [...filtered].sort((a, b) => {
    const so = ORDER[a.status] - ORDER[b.status];
    return so !== 0 ? so : b.value - a.value;
  });
  ...
  const vencidas     = allEntries.filter(e => e.status === "VENCIDO").length;
  ...
  async function handleDelete(id: string) {
    ...
    if (id.startsWith("rec-")) {
      await fetch(`/api/finance/${id.replace(/^rec-/, "")}`, { method: "DELETE" });
    } else {
    ...
    <div style={{ flex: 1, overflowY: "auto", background: "var(--bg-base)" }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 3 }}>Receitas, despesas e lucratividade</p>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ textAlign: "center", minWidth: 148 }}>
            ...
            {isCurrentMonth && (
              <span style={{ marginLeft: 7, fontSize: 10, fontWeight: 600, color: "var(--accent)", background: "var(--accent-soft)", padding: "2px 7px", borderRadius: 20 }}>atual</span>
            )}
          </div>
      ...
      <div style={{ padding: "22px 32px", display: "flex", flexDirection: "column", gap: 22 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
          <KpiCard label="Receita confirmada" value={fmtBRL(totalReceita)} sub={pendReceita > 0 ? `+${fmtBRL(pendReceita)} pendente` : "Tudo recebido"} icon={<TrendingUp size={14} color="#16A34A" />} iconBg="rgba(22,163,74,0.1)" valueColor="#16A34A" />
          <KpiCard label="Despesas pagas"     value={fmtBRL(totalDespesa)} sub={pendDespesa > 0 ? `+${fmtBRL(pendDespesa)} pendente` : "Sem pendências"} icon={<TrendingDown size={14} color="#DC2626" />} iconBg="rgba(220,38,38,0.1)" valueColor="#DC2626" />
          <KpiCard label="Lucro líquido"      value={fmtBRL(lucro)}        sub="receita − despesa (pago)" icon={<Wallet size={14} color={lucro >= 0 ? "var(--accent)" : "#DC2626"} />} iconBg="var(--accent-soft)" valueColor={lucro >= 0 ? "var(--accent)" : "#DC2626"} />
          <KpiCard label="A receber"          value={fmtBRL(pendReceita)}  sub={`${allEntries.filter(e => e.type === "RECEITA" && e.status === "PENDENTE").length} cobrança(s)`} icon={<Clock size={14} color="#D97706" />} iconBg="rgba(217,119,6,0.1)" valueColor="#D97706" />
          <KpiCard label="Vencidas"           value={String(vencidas)}     sub={vencidas > 0 ? "requer atenção" : "Nenhuma em atraso"} icon={<AlertCircle size={14} color={vencidas > 0 ? "#DC2626" : "var(--text-muted)"} />} iconBg={vencidas > 0 ? "rgba(220,38,38,0.1)" : "var(--bg-elevated)"} valueColor={vencidas > 0 ? "#DC2626" : "var(--text-muted)"} alert={vencidas > 0} />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 280px", gap: 20, alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
              ...
              <span style={{ fontSize: 11, color: "var(--text-muted)" }}>
                {sortedFiltered.length} lançamento{sortedFiltered.length !== 1 ? "s" : ""}
              </span>
            </div>

            {/* Table */}
            <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 110px 76px 110px 76px 96px", padding: "9px 18px", borderBottom: "1px solid var(--border)", background: "var(--bg-elevated)" }}>
              ...
              {sortedFiltered.length === 0 ? (
                <div style={{ padding: "48px 0", textAlign: "center" }}>
                  <Wallet size={26} style={{ color: "var(--text-muted)", opacity: 0.18, margin: "0 auto 10px", display: "block" }} />
                  <p style={{ fontSize: 13, color: "var(--text-muted)" }}>Nenhum lançamento neste período</p>
                  <p style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 4, opacity: 0.6 }}>Use os botões acima para registrar receitas ou despesas</p>
                </div>
              ) : (
                sortedFiltered.map((entry, i) => (
                  <EntryRow
                    key={entry.id}
                    entry={entry}
                    last={i === sortedFiltered.length - 1}
                    onDelete={entry.id.startsWith("team-") ? undefined : handleDelete}
                    onOpenStatusModal={() => setStatusModal({ entry })}
              ...
              {/* Footer total */}
              {sortedFiltered.length > 0 && (
                <div style={{ display: "grid", gridTemplateColumns: "1fr 110px 76px 110px 76px 96px", padding: "9px 18px", borderTop: "1px solid var(--border)", background: "var(--bg-elevated)" }}>
                  <span style={{ fontSize: 11, color: "var(--text-muted)", gridColumn: "1 / 4" }}>
                    Saldo do período
                  </span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: "var(--text-primary)" }}>
                    {fmtBRL(sortedFiltered.reduce((s, e) => e.type === "RECEITA" ? s + e.value : s - e.value, 0))}
                  </span>
                </div>
              )}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            {clientProfitability.length > 0 && (
              <SidePanel title="Lucratividade por cliente" icon={<TrendingUp size={11} style={{ color: "#16A34A" }} />}>
                  ...
                  {clientProfitability.map(c => (
                    <div key={c.id}>
                      ...
                      <div style={{ height: 3, borderRadius: 2, background: "var(--bg-elevated)", overflow: "hidden" }}>
                        <div style={{ height: "100%", borderRadius: 2, width: `${c.receita > 0 ? Math.min(100, Math.round((c.receita / (c.receita + c.despesa)) * 100)) : 0}%`, background: c.lucro >= 0 ? "#16A34A" : "#DC2626", opacity: 0.7 }} />
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {/* Recorrentes summary */}
            {(() => {
              const recs = allEntries.filter(e => e.mode === "RECORRENTE");
              const recReceita = recs.filter(e => e.type === "RECEITA").reduce((s, e) => s + e.value, 0);
              const recDespesa = recs.filter(e => e.type === "DESPESA").reduce((s, e) => s + e.value, 0);
              if (recs.length === 0) return null;
              return (
                <SidePanel title="Recorrências do mês" icon={<RefreshCw size={11} style={{ color: "var(--accent)" }} />}>
                  <div style={{ padding: "10px 14px", display: "flex", flexDirection: "column", gap: 7 }}>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Receitas recorrentes</span>
                      <span style={{ fontSize: 12, fontWeight: 700, color: "#16A34A" }}>{fmtBRL(recReceita)}</span>
                    </div>
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Despesas recorrentes</span>
                      <span style={{ fontSize: 12, fontWeight: 700, color: "#DC2626" }}>{fmtBRL(recDespesa)}</span>
                    </div>
                    <div style={{ height: 1, background: "var(--border)", margin: "3px 0" }} />
                    <div style={{ display: "flex", justifyContent: "space-between" }}>
                      <span style={{ fontSize: 12, fontWeight: 600, color: "var(--text-primary)" }}>{recs.length} lançamento{recs.length !== 1 ? "s" : ""}</span>
                      <span style={{ fontSize: 12, fontWeight: 700, color: recReceita - recDespesa >= 0 ? "#16A34A" : "#DC2626" }}>{fmtBRL(recReceita - recDespesa)}</span>
                    </div>
                  </div>
                </SidePanel>
              );
            ...
            {team.filter(p => p.status === "ATIVO").length > 0 && (
              <div style={{ padding: "9px 12px", borderRadius: 9, background: "rgba(37,99,235,0.06)", border: "1px solid rgba(37,99,235,0.15)", fontSize: 11, color: "#2563EB", display: "flex", alignItems: "center", gap: 6 }}>
                <Users size={11} />
                <span><strong>{team.filter(p => p.status === "ATIVO").length}</strong> pessoa(s) incluídas automaticamente da Equipe.</span>
              </div>
            )}
          </div>
        </div>
      ...
      {statusModal && (
        <StatusModal
          ...
          onConfirm={(status) => handleStatusChange(statusModal.entry.id, status)}
        />
      )}
    </div>
  );
...
function EntryRow({ entry, last, onDelete, onOpenStatusModal, isTeam }: {
  ...
  return (
    <div
      ...
      style={{
        display: "grid", gridTemplateColumns: "1fr 110px 76px 110px 76px 96px",
        padding: "11px 18px",
    ...
    >
      ...
      <div style={{ minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {entry.mode === "RECORRENTE" && !isTeam && <span style={{ fontSize: 9, color: "var(--accent)", background: "var(--accent-soft)", padding: "1px 5px", borderRadius: 4, flexShrink: 0 }}>Recorr.</span>}
        </div>
      ...
      <span style={{ fontSize: 11, color: "var(--text-secondary)", background: "var(--bg-elevated)", padding: "2px 7px", borderRadius: 20, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
        {entry.category}
      </span>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
        ...
        {hover && onDelete && (
          <button onClick={() => onDelete(entry.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", padding: 2, display: "flex", alignItems: "center", borderRadius: 4 }}>
            <X size={11} />
          </button>
        )}
      </div>
...
function StatusModal({ entry, onClose, onConfirm }: {
  entry: Entry;
  onClose: () => void;
  onConfirm: (status: Entry["status"]) => void;
}) {
  const [saving, setSaving] = useState(false);
  const isRec = entry.id.startsWith("rec-") || entry.id.startsWith("team-");

  async function pick(s: Entry["status"]) {
    if (s === entry.status) { onClose(); return; }
    setSaving(true);
    await onConfirm(s);
  }
  ...
  return (
    <div
      style={{ position: "fixed", inset: 0, zIndex: 60, background: "rgba(0,0,0,0.35)", backdropFilter: "blur(3px)", display: "flex", alignItems: "center", justifyContent: "center" }}
      onClick={onClose}
    >
      <div
        style={{ background: "var(--bg-surface)", borderRadius: 14, border: "1px solid var(--border)", padding: "20px 22px", width: 340, boxShadow: "0 20px 50px rgba(0,0,0,0.2)" }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 16 }}>
          <div>
            <p style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)", margin: 0 }}>Alterar status</p>
            <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 3 }}>{entry.description}</p>
            {isRec && (
              <p style={{ fontSize: 11, color: "var(--accent)", marginTop: 3, fontWeight: 500 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {(["PAGO", "PENDENTE", "VENCIDO"] as const).map(s => {
            const cfg = STATUS_CONFIG[s];
            const Icon = cfg.icon;
            const isCurrent = entry.status === s;
            return (
              <button
                key={s}
                onClick={() => pick(s)}
                ...
                style={{
                  ...
                  background: isCurrent ? cfg.bg : "var(--bg-elevated)",
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: 13, fontWeight: isCurrent ? 700 : 500, color: isCurrent ? cfg.color : "var(--text-primary)", margin: 0 }}>{cfg.label}</p>
                  <p style={{ fontSize: 11, color: "var(--text-muted)", margin: 0 }}>
                    {s === "PAGO" ? "Pagamento confirmado" : s === "PENDENTE" ? "Aguardando pagamento" : "Prazo expirado"}
                  </p>
                </div>
                {isCurrent && (
                  <span style={{ fontSize: 10, fontWeight: 700, color: cfg.color, background: cfg.bg, padding: "2px 8px", borderRadius: 20, flexShrink: 0 }}>atual</span>
                )}
                {saving && s !== entry.status && <Loader2 size={13} style={{ animation: "spin 1s linear infinite", color: "var(--text-muted)", flexShrink: 0 }} />}
              </button>
            );
          })}
        </div>
...
function CategoryBreakdown({ title, color, entries }: { title: string; color: string; entries: Entry[] }) {
  ...
  return (
    <SidePanel title={title} icon={<Circle size={7} fill={color} color={color} />}>
      {used.length === 0 ? (
        <div style={{ padding: "12px 14px", fontSize: 12, color: "var(--text-muted)" }}>Sem lançamentos</div>
      ) : (
...
function SidePanel({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
      <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--border)", background: "var(--bg-elevated)", display: "flex", alignItems: "center", gap: 7 }}>
        {icon}
...
function NewEntryModal({ type, clients, currentMonth, currentYear, onClose, onSave }: {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    await onSave({
      ...
      date: mode === "RECORRENTE" ? defaultDate : date,
      status: mode === "RECORRENTE" ? "PENDENTE" : status,
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 50, background: "rgba(0,0,0,0.4)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center" }} onClick={onClose}>
      <div style={{ background: "var(--bg-surface)", borderRadius: 16, border: "1px solid var(--border)", padding: "24px 26px", width: 480, boxShadow: "0 24px 64px rgba(0,0,0,0.2)" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ display: "flex", background: "var(--bg-elevated)", borderRadius: 9, padding: 3, border: "1px solid var(--border)", marginBottom: mode === "RECORRENTE" ? 10 : 18 }}>
          {(["RECORRENTE", "AVULSO"] as const).map(m => (
            <button key={m} type="button" onClick={() => handleModeChange(m)} style={{ flex: 1, padding: "7px 0", borderRadius: 7, border: "none", background: mode === m ? "var(--bg-surface)" : "transparent", color: mode === m ? "var(--text-primary)" : "var(--text-muted)", fontSize: 12, fontWeight: mode === m ? 600 : 500, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 5, transition: "all 120ms ease" }}>
              ...
              {m === "RECORRENTE" ? "Recorrente — todo mês" : "Avulso — só este mês"}
            </button>
          ))}
        </div>
        ...
        {mode === "RECORRENTE" && (
          <div style={{ padding: "8px 12px", background: "var(--accent-soft)", border: "1px solid var(--accent)", borderRadius: 8, marginBottom: 14, fontSize: 11, color: "var(--accent)", fontWeight: 500 }}>
            Aparece em todos os meses. Status (Pago/Pendente) é independente por mês.
          </div>
        )}

        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <FormField label="Descrição">
            <input ref={inputRef} value={description} onChange={e => setDescription(e.target.value)} required placeholder={isReceita ? "Ex: Mensalidade — Cliente X" : "Ex: Plataforma de agendamento"} style={inputStyle} />
          </FormField>
          ...
          <FormField label="Vincular ao cliente (opcional)">
...
function KpiCard({ label, value, sub, icon, iconBg, valueColor, alert }: {
  label: string; value: string; sub: string; icon: React.ReactNode;
  iconBg: string; valueColor: string; alert?: boolean;
}) {
  return (
    <div style={{
      background: "var(--bg-surface)",
      border: `1px solid ${alert ? "#DC262644" : "var(--border)"}`,
      borderLeft: alert ? "3px solid #DC2626" : "3px solid transparent",
      ...
      <div>
        <p style={{ fontSize: 20, fontWeight: 800, color: valueColor, letterSpacing: "-0.04em", lineHeight: 1, margin: 0 }}>{value}</p>
