...
export function AuditContent() {
  ...
  function handleReport() {
    if (!clientId) return;
    const qs = `clientId=${clientId}&year=${year}&month=${month}`;
  ...
  if (clients.length === 0) return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 12, padding: 48, textAlign: "center" }}>
      ...
      <p style={{ fontSize: 13, color: "var(--text-muted)", maxWidth: 380, lineHeight: 1.6 }}>
        Nenhum cliente conectado ao Kommo ainda. Abra um cliente → aba <strong>Leads</strong> → conecte o Kommo para começar.
      </p>
    </div>
  );
      ...
      <div style={{ padding: "20px 28px 16px", borderBottom: "1px solid var(--border)", background: "var(--bg-surface)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: "var(--text-muted)", margin: 0 }}>Leads por anúncio, puxados do Kommo</p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
          ...
          <button onClick={handleReport} style={{ ...btn, background: "var(--accent)", color: "#fff", border: "none" }}>
            <FileDown size={13} /> Gerar Relatório
          </button>
        </div>
      </div>
        ...
        {!data || data.groups.length === 0 ? (
          <div style={{ textAlign: "center", padding: "48px 0" }}>
            ...
            <p style={{ fontSize: 14, color: "var(--text-muted)" }}>
              Nenhum lead em {MONTHS[month - 1]} de {year}. Clique em Sincronizar para puxar do Kommo.
            </p>
          </div>
        ) : (
          data.groups.map((g) => (
            <div key={g.adTag} style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: 12, overflow: "hidden" }}>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "12px 16px", background: "var(--bg-elevated)" }}>
                <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-primary)" }}>{g.adTag}</span>
                </span>
                <span style={{ fontSize: 12, fontWeight: 700, color: "var(--accent)", background: "rgba(124,58,237,0.1)", padding: "2px 10px", borderRadius: 20 }}>
                  {g.total} lead{g.total !== 1 ? "s" : ""}
                </span>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1.2fr 1.4fr 90px", padding: "8px 16px", borderBottom: "1px solid var(--border)" }}>
              ...
              {g.leads.map((l, i) => (
                ...
                >
                  ...
                  <span style={{ fontSize: 11, color: "var(--text-muted)" }}>{new Date(l.createdAtKommo).toLocaleDateString("pt-BR", { day: "2-digit", month: "2-digit" })}</span>
                </div>
              ))}
            </div>
          ))
