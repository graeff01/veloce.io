...
// POST — a equipe ASSUME a conversa (silencia a IA na janela, antes da 1ª mensagem).
...
export async function POST(_req: Request, { params }: { params: Promise<{ token: string; contactId: string }> }) {
  ...
  if (await isProtected(portal.clientId)) {
    ...
    if (!email) return NextResponse.json({ error: "Faça login para assumir a conversa." }, { status: 401 });
  }
