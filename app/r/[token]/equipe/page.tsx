...
export default async function EquipePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <main className="tmain">
      ...
      <PortalShell token={token} brandName={client?.name || "Painel"} logoUrl={client?.logoUrl ?? null} active="equipe" sections={shell.sections} account={shell.account} aiTest={shell.aiTest} />
