...
export default async function FechamentoPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <main className="fmain">
      ...
      <PortalShell token={token} brandName={client?.name || "Painel"} logoUrl={client?.logoUrl ?? null} active="fechamento" sections={shell.sections} account={shell.account} aiTest={shell.aiTest} />
