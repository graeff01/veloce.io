...
export default async function OrcamentosPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  // Aba disponível só pra clientes com orçamento habilitado (ex.: JR).
  const ai = await prisma.aiAgentConfig.findUnique({ where: { clientId: portal.clientId }, select: { quotesEnabled: true } });
  if (!ai?.quotesEnabled) redirect(`/r/${token}/conversas`);
  ...
  return (
    <main className="qmain">
      ...
      <PortalShell token={token} brandName={client?.name || "Painel"} logoUrl={client?.logoUrl ?? null} active="orcamentos" />
      <style>{`${themeSwitchCss(portal.accentColor, portal.mode)} ${PORTAL_UI_CSS} *{box-sizing:border-box}
        .qmain{min-height:100dvh;color:var(--p-text);font-family:system-ui,-apple-system,sans-serif;background:var(--p-bg)}
        @media(min-width:1024px){ .qmain{margin-left:236px} }`}</style>
