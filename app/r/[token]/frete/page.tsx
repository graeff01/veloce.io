...
export default async function FretePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <main className="fmain">
      ...
      <PortalShell token={token} brandName={client?.name || "Painel"} logoUrl={client?.logoUrl ?? null} active="frete" sections={shell.sections} account={shell.account} aiTest={shell.aiTest} />
