...
export default async function TestarIaPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return (
    <main className="imain">
      ...
      <PortalShell token={token} brandName={client?.name || "Painel"} logoUrl={client?.logoUrl ?? null} active="teste" sections={shell.sections} account={shell.account} aiTest={shell.aiTest} quotesEnabled={shell.quotesEnabled} />
