...
export interface PortalState { token: string; link: string; accentColor: string | null; mode: string; active: boolean }
...
export async function getOrCreatePortal(clientId: string): Promise<PortalState> {
  ...
  return { token: portal.token, link: `${APP_URL}/r/${portal.token}`, accentColor: portal.accentColor, mode: portal.mode, active: portal.active };
}
...
export async function updatePortal(clientId: string, data: { accentColor?: string | null; mode?: string; active?: boolean }): Promise<void> {
