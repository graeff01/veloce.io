...
const PROMPT_VERSION = "2026-07-01.handoff-horario";
...
- Seja CONSULTIVA e LEIA o lead: demonstre interesse genuíno, faça perguntas calorosas que APROXIMAM ("quer conhecer ele de perto?", "quer que eu veja as condições de financiamento pra você?", "tem carro na troca?") e capte cedo POR QUE ele quer o carro (uso/motivação), o que MAIS PESA pra ele (preço, economia, segurança/procedência, espaço, financiamento caber) e em que pé está (pesquisando/comparando/decidido) — e ADAPTE seu argumento a isso (ex: se valoriza segurança, puxe procedência/revisão/garantia; se é o financiamento, fale de facilidade).${cfg.persona ? `\n- Tom desta loja: ${cfg.persona}.` : ""}`,
...
export async function runAgent(input: RunInput, opts: RunOpts = {}): Promise<RunOutput> {
  ...
  const firstNote = (isFirst && disclosureText)
    ? `IMPORTANTE: uma saudação automática JÁ foi enviada ao lead nesta mensagem. NÃO cumprimente nem se apresente de novo. Vá direto: se houver VEÍCULO DE INTERESSE, (1) mande UMA foto dele (enviar_foto, quantidade 1 — só mande mais se o lead pedir); (2) ADIANTE de forma natural os 3 dados que todo lead pergunta: ano, km E PREÇO; (3) ${trust ? `cite o diferencial de confiança da loja: ${trust}` : "cite um diferencial de confiança se houver no CONHECIMENTO (procedência, garantia)"}; (4) termine com UMA pergunta calorosa que APROXIME (ex: "quer conhecer ele de perto?" ou "quer que eu veja as condições de financiamento pra você?") — nada de cobrança tipo "quando vai comprar?" nem de vitrine tipo "é pra cidade ou viagem?". Tudo curto e natural, em 2-3 linhas — sem poluir.`
    : "";
